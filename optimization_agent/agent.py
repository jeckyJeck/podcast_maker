...
try:
    from .agent_llm import AgentLLM, build_agent_llm, PROVIDERS
...
AGENT_SYSTEM_PROMPT = """You are an autonomous improvement agent for a podcast production pipeline.

Your goal is to improve podcast quality by iterating on prompt files and pipeline YAML configuration. You never generate podcast content directly. Content generation happens only when you call `run_pipeline`, which invokes `engine.py` and Gemini.

Available actions and required params (types + example values):

- `read_file` (params: object)
    - `path` (string) — relative path under the agent root. Example: {"path": "prompts/architect-duo-long.md"}

- `write_file` (params: object)
    - `path` (string) — relative path to write. Example: {"path": "prompts/researcher.md"}
    - `content` (string) — full file contents to write. Example: {"content": "# New prompt..."}

- `list_runs` (params: object)
    - (no params required) Example: {}

- `read_run` (params: object)
    - `run_id` (string) — run directory name. Example: {"run_id": "run_001"}

- `read_run_full` (params: object)
    - `run_id` (string). Example: {"run_id": "run_002"}

- `diff_files` (params: object)
    - `path_a` (string), `path_b` (string). Example: {"path_a": "prompts/old.md", "path_b": "prompts/new.md"}

- `diff_runs` (params: object)
    - `run_id_a` (string), `run_id_b` (string), `stage` (string, optional, default "script"). Example: {"run_id_a": "run_010", "run_id_b": "run_011", "stage": "outline"}

- `run_pipeline` (params: object)
    - `topic` (string), `format` (string), `extra_inputs` (object, optional). Example: {"topic": "quantum computing", "format": "solo_short", "extra_inputs": {"guest": "Dr. X"}}

- `run_evaluation` (params: object)
    - `run_id` (string). Example: {"run_id": "run_012"}

- `run_all_test_topics` (params: object)
    - `extra_inputs` (object, optional). Example: {"extra_inputs": {"seed": 42}}

- `log_experiment` (params: object)
    - `name` (string), `hypothesis` (string), `changes_made` (array of strings), `results` (object), `conclusion` (string).
        Example: {"name": "iteration_003", "hypothesis": "shorter intro improves focus", "changes_made": ["prompts/scriptwriter.md"], "results": {}, "conclusion": "TBD"}

- `revert_file` (params: object)
    - `path` (string), `run_id` (string). Example: {"path": "prompts/architect-duo-long.md", "run_id": "run_005"}

Rules for constructing tool calls:
- Always return ONLY valid JSON (no markdown fences) with this exact top-level shape:
    {
        "reasoning": "why this action is next",
        "action": "tool_name",
        "params": { ... },
        "done": false
    }
- `params` MUST be an object. Use the exact parameter names shown above. Types must match (strings, objects, arrays).
- If a tool has no parameters, return an empty object for `params` (e.g. `{}`).
- When calling `write_file`, include the complete file `content` (not a diff).
- When calling `run_pipeline`, prefer `extra_inputs` as an object of simple scalars (strings/numbers/booleans).
...
6. Mark done when the current iteration has a clear conclusion.

Pipeline YAML changes are allowed, including adding/removing stages, changing execution types, and adding user_inputs with matching prompt updates.

If done is true, still include action and params (they may be empty). No prose outside the JSON. No markdown fences.
"""
...
def ask_agent(agent_llm: AgentLLM, context: str, stream_thoughts: bool = False) -> dict[str, Any]:
    response_text = agent_llm.complete(AGENT_SYSTEM_PROMPT, context, stream_thoughts=stream_thoughts)
    try:
...
def run_smoke_test(format_filter: str | None, agent_llm: AgentLLM, stream_thoughts: bool = False) -> None:
    ...
    action = ask_agent(
        agent_llm,
        """
Run exactly one safe tool call for a smoke test. Choose a read-only action from:
- read_file
- list_runs
- read_run
- read_run_full
