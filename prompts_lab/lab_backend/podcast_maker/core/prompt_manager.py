...
class PromptManager:
    ...
    def get_researcher_prompt(self, direction: Dict[str, Any], previous_research: str = "") -> str:
        ...
        direction_json_string = json.dumps(direction, ensure_ascii=False, indent=2)
        
        # Optional: extract these for display/override purposes
        target_word_count = direction.get("target_word_count", "N/A")
        depth_level = direction.get("depth_level", "MEDIUM")
        
        # Replace placeholders
        prompt = prompt.replace("{{DIRECTION_JSON}}", direction_json_string)
        prompt = prompt.replace("{{TARGET_WORD_COUNT}}", str(target_word_count))
        prompt = prompt.replace("{{DEPTH_LEVEL}}", str(depth_level))
        prompt = prompt.replace("{{PREVIOUS_RESEARCH}}", previous_research)
        return prompt
