"""
Pipeline YAML Contract Validator.
"""
...
def _validate_prompt(prompt: Any, stage_path: str, root: Path, issues: List[ValidationIssue]) -> Optional[str]:
    ...
    if source == "file":
        ...
        if not isinstance(prompt_path, str) or not prompt_path.strip():
            ...
            return None
        # Resolve relative to prompts_lab (root.parent) or backend root (root)
        resolved = root / prompt_path
        resolved_alt = root.parent / prompt_path
        
        # Try both locations
        target_path = resolved if resolved.is_file() else resolved_alt
...
def _validate_bindings(
    ...
    for index, binding in enumerate(bindings):
        ...
        if not isinstance(placeholder, str) or not placeholder:
            ...
            # Note: Stored templates or files might not be resolved yet, but if prompt_text is loaded, warn/raise
...
def _validate_response(response: Any, stage_path: str, execution: str, root: Path, issues: List[ValidationIssue]) -> None:
    ...
    if schema is not None:
        if kind != "json":
            ...
            resolved_alt = root.parent / schema
            target_path = resolved if resolved.is_file() else resolved_alt
            if not target_path.is_file():
