...
MALE_VOICE_CHIRPHD = "en-US-Chirp3-HD-Algieba"#"en-US-Chirp-HD-D"
FEMALE_VOICE_CHIRPHD = "en-US-Chirp3-HD-Aoede"# "en-US-Chirp-HD-O"
...
class GoogleTTS:
    ...
    def create_full_conversation(self, dialogue_lines):
        """
        ...
        print(f"Done generating audio for conversation. Total characters: {char_used}")
        return combined_audio

    def split_script_into_lines(self, script, voice_dict):
        """
        Splits the script into lines.
        Returns a list of tuples: (text_chunk, voice_name)
        """
        lines = []
        ...
        for line in script.split("\n"):
            ...
            if match:
                ...
                lines.append((text.strip(), voice_name))
            else:
    ...
    def text_to_speech_not_SSML(self, script: str, voice_dict: dict) -> bytes:
        lines = self.split_script_into_lines(script, voice_dict)
        return self.create_full_conversation(lines)

    def format_to_SSML(self, script: str, voice_dict: dict) -> str:
...
<break time="300ms"/>
    ...
    def text_to_speech_SSML(self, script: str, voice_dict: dict) -> bytes:
...
if __name__ == "__main__":
  ...
  """
    ...
    # IMPORTANT: Use the 'ssml' parameter, NOT 'text'
    input_text = texttospeech.SynthesisInput(ssml=sample_ssml)

    # Step 4: Configure the voice fallback
    # Note: The 'name' here acts as a default, but the SSML tags will override it
    voice = texttospeech.VoiceSelectionParams(
        language_code="en-US",
        name=MALE_VOICE_CHIRPHD
    )
